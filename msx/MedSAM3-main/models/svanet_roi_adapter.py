...
def select_small_triggers(
    ...
    if mode == "all":
        return torch.ones_like(predicted)
    if mode == "pred":
        return predicted
...
class SvANetROIAdapter(nn.Module):
    ...
    def __init__(
        ...
        train_trigger: str = "teacher_forcing",
        prefer_prompt_roi: bool = False,
    ) -> None:
        super().__init__()
        if empty_mask_fallback != "box_then_full_image":
            raise ValueError("Only empty_mask_fallback=box_then_full_image is supported")
        if paste_mode not in {"replace_roi", "blend_with_sam3", "soft_gate"}:
            raise ValueError(
                "paste_mode must be replace_roi, blend_with_sam3 or soft_gate"
            )
        ...
        self.prefer_prompt_roi = bool(prefer_prompt_roi)
    ...
    def forward(
        ...
        small_reference = (
            area_labels.to(area_logits.device, dtype=torch.long) == 0
            if self.training and area_labels is not None
            else area_logits.argmax(dim=-1) == 0
        )
        # Keep the gate differentiable even when the expensive SvANet execution
        # is conditionally selected. During training, the final-mask loss can
        # therefore improve both the area router and the refinement expert.
        small_probabilities = area_logits.float().softmax(dim=-1)[:, 0]
        ...
        for index in trigger_indices:
            ...
            if (
                self.prefer_prompt_roi
                and not use_gt_roi
                and prompt is not None
            ):
                box, source = tuple(prompt), "box_prompt"
            else:
                source_mask = (
                    gt_masks[index].bool()
        ...
        fusion_gates = torch.zeros_like(small_probabilities)
        ...
        if roi_images:
            ...
            for local_index, image_index in enumerate(trigger_indices):
                ...
                if self.paste_mode == "replace_roi":
                    ...
                    fusion_gates[image_index] = 0.5
                else:
                    gate = small_probabilities[image_index].to(
                        device=final_logits.device, dtype=final_logits.dtype
                    )
                    base = final_logits[image_index, y1:y2, x1:x2]
                    final_logits[image_index, y1:y2, x1:x2] = (
                        base + gate * (resized - base)
                    )
                    fusion_gates[image_index] = gate
                roi_logits_list.append(roi_logits[local_index])
            if gt_masks is not None:
                target_batch = torch.stack(roi_targets)
                refine_selection = torch.ones(
                    len(trigger_indices),
                ...
                if self.training and area_labels is not None:
                    refine_selection = (
                        area_labels.to(roi_logits.device, dtype=torch.long)[
                            trigger_indices
                        ]
                        == 0
                    )
                refine_loss = (
                    dice_bce_with_logits(
                        roi_logits[refine_selection],
                        target_batch[refine_selection],
                        debug_label="refine",
                    )
                    if bool(refine_selection.any())
                    else self._zero_refine_loss(images)
                )
            else:
        ...
        return {
            ...
            "batch_stats": {
                ...
                "small_count": int(small_reference.sum().item()),
                ...
                "mean_fusion_gate": (
                    float(fusion_gates[trigger].detach().mean().item())
                    if bool(trigger.any()) else 0.0
                ),
            },
        }
