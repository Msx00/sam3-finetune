...
STAGE_ACTIVE_LOSSES = {
    1: {"native_sam3_loss", "sam3_loss", "aux_loss"},
    2: {
        "aux_loss", "modality_loss", "area_loss", "area_reg_loss",
        "boundary_router_loss",
    },
    3: {
        "native_sam3_loss", "sam3_loss", "boundary_seg_loss",
        "load_balance_loss",
    },
    4: {"refine_loss", "final_fusion_loss"},
    5: {
        "native_sam3_loss", "sam3_loss", "aux_loss", "modality_loss", "area_loss",
        "area_reg_loss", "boundary_router_loss", "boundary_seg_loss",
        "load_balance_loss", "refine_loss", "final_fusion_loss",
    },
}
...
class StageTrainingManager:
    ...
    def trainable_group_names(self) -> Set[str]:
        policy = {
            ...
            3: {"expert_lora"},
            4: {"svanet"},
            5: {"router", "auxiliary_head", "expert_lora", "svanet"},
        }[self.stage]
        ...
        if self.stage == 5 and self.stage_config.get("train_decoder_stage5", False):
            policy.add("decoder")
        return policy
    ...
    def save_checkpoint(
        ...
        payload = {
            ...
            "best_metric_mode": str(best_metric_mode),
    ...
    def load_checkpoint(
        ...
        if payload.get("controller_state"):
            self.controller.load_state_dict(payload["controller_state"], strict=True)
        if self.svanet_adapter is not None and payload.get("svanet_state"):
            self.svanet_adapter.load_state_dict(payload["svanet_state"], strict=True)
        return payload
