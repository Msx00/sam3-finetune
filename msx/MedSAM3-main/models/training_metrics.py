...
LOSS_NAMES = (
    "total_loss", "native_sam3_loss", "sam3_loss", "aux_loss",
    "modality_loss", "area_loss", "area_reg_loss", "boundary_router_loss",
    "boundary_seg_loss", "load_balance_loss", "refine_loss",
    "final_fusion_loss",
)
...
class EpochStatistics:
    ...
    def update_svanet(self, output: Optional[Mapping[str, Any]]) -> None:
        ...
        for name in (
            "sample_count", "small_count", "trigger_count", "empty_mask_count",
        ...
        self.svanet["fusion_gate_weighted"] += (
            float(stats.get("mean_fusion_gate", 0)) * trigger_count
        )
    ...
    def report(self, configured_teacher_ratio: float = 0.0) -> Dict[str, Any]:
        ...
        svanet = {
            "sample_count": int(self.svanet["sample_count"]),
            ...
            "mean_fusion_gate": (
                self.svanet["fusion_gate_weighted"] / triggers if triggers else 0.0
            ),
