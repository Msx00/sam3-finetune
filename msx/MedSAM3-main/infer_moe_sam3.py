...
def _checkpoint_runtime_metadata(checkpoint: str) -> Dict[str, Any]:
    try:
        payload = torch.load(
    ...
    metadata = {
        ...
        "area_thresholds": copy.deepcopy(payload.get("area_thresholds") or {}),
        "boundary_thresholds": copy.deepcopy(
            payload.get("boundary_thresholds") or {}
        ),
    }
    del payload
    gc.collect()
...
def build_runtime(config_path: str, checkpoint: str, stage: int | None, device: int):
    ...
    metadata = _checkpoint_runtime_metadata(checkpoint)
    config = effective_inference_config(
        external_config, metadata["config"]
    )
    ...
    selected_stage = int(
        ...
        if stage is not None
            ...
            else config.get("training", {}).get("stage", 5)
        )
    )
    config.setdefault("training", {})["stage"] = selected_stage
    ...
    loaded = trainer.stage_manager.load_checkpoint(
        checkpoint, allowed_stages={selected_stage}
    )
    trainer.effective_area_thresholds = (
        loaded.get("area_thresholds") or metadata["area_thresholds"] or None
    )
    trainer.effective_boundary_thresholds = (
        loaded.get("boundary_thresholds")
        or metadata["boundary_thresholds"]
        or None
    )
