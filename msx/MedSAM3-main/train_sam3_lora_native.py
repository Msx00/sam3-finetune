...
class SAM3TrainerNative:
    def __init__(
        ...
        if config_data is None:
            ...
            self.config = copy.deepcopy(dict(config_data))
        training_overrides = dict(training_overrides or {})
        if "batch_size" in training_overrides:
            batch_size = int(training_overrides["batch_size"])
            if batch_size <= 0:
                raise ValueError("batch_size override must be positive")
            self.config.setdefault("training", {})["batch_size"] = batch_size
        if "epochs" in training_overrides:
            epochs = int(training_overrides["epochs"])
            if epochs <= 0:
                raise ValueError("epochs override must be positive")
            self.config.setdefault("training", {})["epochs"] = epochs
            self.config["training"]["num_epochs"] = epochs
        dataset_config = dict(
            patient_dataset_config or self.config.get("dataset") or {}
        )
        for key, value in dict(dataset_overrides or {}).items():
            count = int(value)
            if count <= 0:
                raise ValueError(f"{key} override must be positive")
            dataset_config[key] = count
        self.patient_dataset_config = dataset_config
        self.config["dataset"] = dict(dataset_config)
        self.router_config = dict(router_config or {})
        self.svanet_config = dict(svanet_config or {})
        self.end_to_end = bool(end_to_end)
        moe_config = None if moe_config is None else dict(moe_config)
        if moe_config is not None:
            # Backward compatibility for older YAMLs that kept the MoE
            # injection scope under model.* instead of moe.*.
            model_settings = self.config.get("model", {}) or {}
            legacy_moe_fields = {
                "decoder_layers": "moe_decoder_layers",
                "target_modules": "moe_target_modules",
                "rank": "expert_lora_rank",
                "alpha": "expert_lora_alpha",
                "dropout": "expert_lora_dropout",
            }
            for destination, source in legacy_moe_fields.items():
                if destination not in moe_config and source in model_settings:
                    moe_config[destination] = model_settings[source]
        if self.end_to_end:
            # One-stage PEFT: keep the pretrained SAM3 base frozen, but jointly
            # train shared LoRA, routed LoRA experts, routers and SvANet
            # and the differentiable fusion path from the first epoch.
            training_stage = 5
            self.config.setdefault("training", {})["stage"] = 5
            self.config["training"]["end_to_end"] = True
            self.config.setdefault("stages", {})[
                "train_shared_lora_stage5"
            ] = True
            self.config["stages"].setdefault("train_decoder_stage5", False)
            if moe_config is None:
                raise ValueError("end_to_end training requires moe.enabled=true")
            moe_config["routing_mode"] = "soft"
            self.router_config["routing_type"] = "soft"
            self.router_config["use_gt_modality_during_train"] = False
            self.router_config["use_gt_area_during_warmup"] = False
            self.router_config["use_gt_boundary_during_warmup"] = False
            self.router_config["teacher_forcing"] = {
                "enabled": False,
                "start_ratio": 0.0,
                "end_ratio": 0.0,
                "decay_epochs": 1,
            }
            # Execute both output experts during training and mix them with a
            # soft gate. Inference still uses the predicted hard small-target
            # trigger, so easy slices bypass SvANet.
            self.svanet_config["train_trigger"] = "all"
            self.svanet_config["paste_mode"] = "soft_gate"
            self.svanet_config["use_gt_roi_for_warmup"] = False
            self.svanet_config["prefer_prompt_roi"] = True
            load_stage_dependencies = False
            print_rank0(
                "End-to-end mode: stage=5, routing=soft, "
                "shared LoRA/router/experts/SvANet/fusion=trainable; "
                "SAM3 base decoder frozen"
            )
        self.config["router"] = copy.deepcopy(self.router_config)
        self.config["svanet"] = copy.deepcopy(self.svanet_config)
        if moe_config is not None:
            self.config["moe"] = copy.deepcopy(moe_config)
        ...
        self.start_epoch = 0
        self.checkpoint_metric_name = str(
            self.config.get("training", {}).get(
                "checkpoint_metric", "val_loss"
            )
        )
        self.checkpoint_metric_mode = str(
            self.config.get("training", {}).get(
                "checkpoint_metric_mode",
                "min" if self.checkpoint_metric_name == "val_loss" else "max",
            )
        ).lower()
        if self.checkpoint_metric_mode not in {"min", "max"}:
            raise ValueError("training.checkpoint_metric_mode must be min or max")
        ...
        self.effective_area_thresholds = None
        self.effective_boundary_thresholds = None
        ...
        if self.moe_controller is not None:
            ...
            component_weights = {
                "native_sam3_loss": float(
                    loss_config.get("lambda_native_sam3", 1.0)
                ),
                ...
                "final_fusion_loss": float(
                    loss_config.get("lambda_final_fusion", 1.0)
                ),
            }
            boundary_kernel = int(
                loss_config.get(
        ...
        if self.training_stage is not None:
            ...
            if self.training_stage in {4, 5}:
                ...
                self.svanet_adapter = SvANetROIAdapter(
                    ...
                    prefer_prompt_roi=bool(
                        self.svanet_config.get("prefer_prompt_roi", False)
                    ),
                ).to(self.device)
        ...
        if configured_resume:
            ...
            resumed_metric_name = str(
                resumed.get("best_metric_name", "val_loss")
            )
            resumed_metric_mode = str(
                resumed.get("best_metric_mode", "min")
            )
            if (
                resumed_metric_name != self.checkpoint_metric_name
                or resumed_metric_mode != self.checkpoint_metric_mode
            ):
                raise ValueError(
                    "Resume checkpoint metric is incompatible with the current "
                    f"configuration: checkpoint={resumed_metric_name}/"
                    f"{resumed_metric_mode}, current={self.checkpoint_metric_name}/"
    ...
    def _add_moe_aux_loss(
        ...
        total_loss, components = self.moe_loss(
            ...
            native_sam3_loss=native_sam3_loss,
        )
        if final_logits.shape[0] > 0 and input_batch is not None:
            ...
            if self.svanet_adapter is not None:
                ...
                from models.moe_losses import dice_bce_with_logits

                final_fusion_loss = dice_bce_with_logits(
                    adapter_output["final_logits"],
                    gt_masks,
                    debug_label="final_fusion",
                )
                total_loss = (
                    total_loss
                    + self.moe_loss.weights.get("refine_loss", 0.0) * refine_loss
                    + self.moe_loss.weights.get("final_fusion_loss", 0.0)
                    * final_fusion_loss
                )
                components["refine_loss"] = refine_loss
                components["final_fusion_loss"] = final_fusion_loss
    ...
    def _save_stage_checkpoint(self, out_dir, epoch, loss, best=False):
        ...
        self.stage_manager.save_checkpoint(
            ...
            epoch=epoch,
            best_loss=loss,
            best_metric_name=self.checkpoint_metric_name,
            best_metric_mode=self.checkpoint_metric_mode,
    ...
    def _wandb_report_metrics(prefix, report):
        ...
        sample_count = float(svanet.get("sample_count", 0))
        trigger_count = float(svanet.get("trigger_count", 0))
        metrics[f"{prefix}/svanet/trigger_ratio"] = (
            trigger_count / sample_count if sample_count else 0.0
        )
    ...
    def _validation_checkpoint_metric(self, avg_val_loss, val_report):
        if self.checkpoint_metric_name == "val_loss":
            return float(avg_val_loss)
        segmentation = val_report.get("segmentation", {}) or {}
        if self.checkpoint_metric_name not in segmentation:
            raise KeyError(
                f"Unknown training.checkpoint_metric "
                f"{self.checkpoint_metric_name!r}; available segmentation "
                f"metrics: {sorted(segmentation)}"
            )
        value = float(segmentation[self.checkpoint_metric_name])
        if not np.isfinite(value):
            raise FloatingPointError(
                f"Non-finite checkpoint metric "
                f"{self.checkpoint_metric_name}: {value}"
            )
        return value

    def _is_better_checkpoint_metric(self, candidate, best):
        if self.checkpoint_metric_mode == "min":
            return candidate < best
        return candidate > best

    def _compute_and_apply_training_thresholds(self, datasets):
        """Derive pseudo-label thresholds from the selected training patients."""
        from data.area_labels import compute_area_thresholds, save_area_thresholds
        ...
        mr_patient_ids = []
        us_patient_ids = []
        boundary_scores = {"mr": [], "us": []}
        scores_by_mask = {}
        boundary_mode = cfg.get("boundary_threshold_mode", {}) or {}
        band_width = int(boundary_mode.get("boundary_band_width", 3))
        for dataset in datasets:
            if dataset.modality == "MR":
                mr_patient_ids.extend(dataset.patient_ids)
            else:
                us_patient_ids.extend(dataset.patient_ids)
            modality = dataset.modality.lower()
            for record in dataset.records:
                ratios.append(record.area_ratio)
                if record.mask_path is None:
                    raise ValueError(
                        "Area/boundary routing requires independent training masks: "
                        f"{record.relative_file_name}"
                    )
                score = compute_boundary_scores(
                    record.image_path, record.mask_path, band_width
                )
                boundary_scores[modality].append(score)
                scores_by_mask[record.mask_path] = score

        area_thresholds, area_counts = compute_area_thresholds(
            ratios,
            cfg.get("area_threshold_mode", {"type": "quantile"}),
            mr_patient_ids,
            us_patient_ids,
        )
        boundary_thresholds, boundary_counts = compute_boundary_thresholds(
            boundary_scores,
            contrast_quantile=float(
                boundary_mode.get("contrast_quantile", 0.33)
            ),
            complexity_quantile=float(
                boundary_mode.get("complexity_quantile", 0.67)
            ),
            boundary_band_width=band_width,
        )
        self.effective_area_thresholds = area_thresholds
        self.effective_boundary_thresholds = boundary_thresholds
        for dataset in datasets:
            dataset.apply_thresholds(
                area_thresholds,
                boundary_thresholds,
                boundary_scores_by_mask=scores_by_mask,
            )

        output_dir = Path(self.config["output"]["output_dir"])
        area_path = Path(
            cfg.get("area_threshold_file", output_dir / "area_thresholds.json")
        )
        boundary_path = Path(
            cfg.get(
                "boundary_threshold_file",
                output_dir / "boundary_thresholds.json",
            )
        )
        cfg["area_threshold_file"] = str(area_path)
        cfg["boundary_threshold_file"] = str(boundary_path)
        self.config["dataset"] = copy.deepcopy(cfg)
        if is_main_process():
            save_area_thresholds(area_thresholds, area_path)
            save_boundary_thresholds(boundary_thresholds, boundary_path)
            print(
                "Computed routing thresholds from selected TRAIN patients: "
                f"area={area_counts}, boundary={boundary_counts}"
            )
    ...
    def _build_patient_dataset(self, split, training):
        ...
        area_thresholds = (
            None
            if training
            else self.effective_area_thresholds or cfg.get("area_threshold_file")
        )
        boundary_thresholds = (
            None
            if training
            else self.effective_boundary_thresholds
            or cfg.get("boundary_threshold_file")
        )
        ...
        if training:
            ...
            self._compute_and_apply_training_thresholds(datasets)
        elif is_validation:
    ...
    def train(self):
        ...
        best_checkpoint_metric = self.resume_best_metric
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            if run_validation:
                ...
                checkpoint_metric = self._validation_checkpoint_metric(
                    avg_val_loss, val_report
                )
                is_new_best = self._is_better_checkpoint_metric(
                    checkpoint_metric, best_checkpoint_metric
                )
                if is_new_best:
                ...
                if is_new_best:
                    epoch_metrics.update({
                        "best/epoch": epoch + 1,
                        f"best/{self.checkpoint_metric_name}": checkpoint_metric,
                    })
                ...
                # Save models based on the configured validation metric.
                if is_main_process():
                    ...
                    if is_new_best:
                        ...
                        print(
                            ...
                            f"({self.checkpoint_metric_name}: "
                            f"{checkpoint_metric:.6f})"
                        )
        ...
        if is_main_process():
            has_best_metric = (
                np.isfinite(best_checkpoint_metric)
            )
            if has_best_metric:
                print(f"\n{'='*80}")
                ...
                print(f"{'='*80}")
                print(
                    f"Best validation {self.checkpoint_metric_name}: "
                    f"{best_checkpoint_metric:.6f}"
                )
                print(f"\nModels saved to {out_dir}:")
                print(
                    f"  - best_lora_weights.pt "
                    f"(best validation {self.checkpoint_metric_name})"
                )
...
def launch_distributed_training(args):
    ...
    for option, attribute in (
        ...
        ("--epochs", "epochs"),
        ("--num-mr-train-cases", "num_mr_train_cases"),
        ("--num-us-train-cases", "num_us_train_cases"),
        ("--num-mr-val-cases", "num_mr_val_cases"),
        ("--num-us-val-cases", "num_us_val_cases"),
    ):
