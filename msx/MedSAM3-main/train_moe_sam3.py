...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Override training.batch_size from the YAML",
    )
    parser.add_argument(
        "--epochs", type=int, default=None,
        help="Override training.epochs/num_epochs from the YAML",
    )
    parser.add_argument("--num-mr-train-cases", type=int, default=None)
    parser.add_argument("--num-us-train-cases", type=int, default=None)
    parser.add_argument("--num-mr-val-cases", type=int, default=None)
    parser.add_argument("--num-us-val-cases", type=int, default=None)
    parser.add_argument(
        "--end-to-end",
        action="store_true",
        help=(
            "Start directly in stage 5 and jointly train shared LoRA, routed "
            "experts, routers, SvANet and soft fusion while keeping the SAM3 "
            "base frozen"
        ),
    )
...
def main():
    ...
    training_stage = int(
        5 if args.end_to_end else (
            args.stage
            if args.stage is not None
            else config.get("training", {}).get("stage", 1)
        )
    )
    ...
    trainer = SAM3TrainerNative(
        ...
        dataset_overrides={
            key: value
            for key, value in {
                "num_mr_patients": args.num_mr_train_cases,
                "num_us_patients": args.num_us_train_cases,
                "num_mr_val_patients": args.num_mr_val_cases,
                "num_us_val_patients": args.num_us_val_cases,
            }.items()
            if value is not None
        },
    )
