...
from models.moe_lora import ExpertPool, RoutedMoELinear
...
class _Decoder(nn.Module):
    def __init__(self, dim):
        ...
        self.d_model = dim
        self.dac = False
        self.layers = nn.ModuleList([_DecoderLayer(dim) for _ in range(6)])
...
class _Model(nn.Module):
    def __init__(self, dim):
        ...
        self.transformer.decoder = _Decoder(dim)


def test_moe_injection_respects_configured_layers_and_projections():
    model = _Model(8)
    ...
    for layer_number, layer in enumerate(model.transformer.decoder.layers, 1):
