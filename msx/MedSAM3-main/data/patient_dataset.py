...
class PatientDataset(Dataset):
    ...
    def apply_thresholds(
        self,
        area_thresholds: Dict[str, Any],
        boundary_thresholds: Dict[str, Any],
        boundary_scores_by_mask: Optional[Dict[str, Any]] = None,
    ) -> None:
        """Apply train-derived thresholds to already selected slice records."""
        self.area_thresholds = dict(area_thresholds)
        self.boundary_thresholds = dict(boundary_thresholds)
        modality_thresholds = self.boundary_thresholds[self.modality.lower()]
        band_width = int(self.boundary_thresholds.get("boundary_band_width", 3))
        updated: List[PatientSliceRecord] = []
        for record in self.records:
            ...
            boundary_fallback = record.boundary_fallback
            boundary_label = -1
            if record.mask_path is not None:
                scores = (boundary_scores_by_mask or {}).get(record.mask_path)
                if scores is None:
                    scores = compute_boundary_scores(
                        record.image_path, record.mask_path, band_width
                    )
                boundary_contrast = scores.contrast
                boundary_complexity = scores.complexity
                boundary_fallback = scores.used_fallback
                boundary_label = boundary_label_from_scores(
                    boundary_contrast,
                    boundary_complexity,
                    float(modality_thresholds["contrast_low"]),
