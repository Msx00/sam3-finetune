...
def _wrap_attention(attention: nn.Module, controller: HierarchicalMoEController) -> int:
    replaced = 0
...
def inject_hierarchical_moe(
    ...
    """Inject routing only into attention projections of decoder layers 4--6."""
    if hasattr(model, "moe_controller"):
        raise RuntimeError("Model already contains a hierarchical MoE controller")
    decoder = model.transformer.decoder
    if len(decoder.layers) < 6:
        raise ValueError("Hierarchical LoRA-MoE requires at least six decoder layers")
    ...
    for layer_idx in range(3, 6):
        ...
        for attention_name in ("self_attn", "ca_text", "cross_attn"):
            attention = getattr(layer, attention_name, None)
            if attention is not None:
                replaced += _wrap_attention(attention, controller)
    controller.install_hooks(decoder)
    if verbose:
        names = list(controller.expert_pool.expert_names)
        print(
            f"Installed Hierarchical LoRA-MoE on decoder layers 4-6: "
